...
\begin{frame}[t]{H3 and H5 --- prognosis is where learning pays}
  ...
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
      \centering
      \includegraphics[width=\linewidth]{rul_distribution}
    \end{column}
    \begin{column}{0.42\textwidth}
      ...
      The traditional errors are wide and \alert{skewed optimistic} --- late
      retirement predictions, the dangerous direction. The AI's are tight and
      near-unbiased.
      \medskip

      \textbf{H5}: conformal intervals cover $88.3\,\%$ at $\pm1957$ cycles
      versus the slope band's over-covering $98.3\,\%$ at $\pm11\,509$ ---
      \alert{a maintenance window six times narrower, with honest stated
      confidence}.
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[t]{The split verdict}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}\small
        \item prognosis: $858$ vs $1981$ cycles, robust across three architectures
          and across sensor quality
  ...
  \begin{alertblock}{And physics did not lose --- it diagnosed the limits}
    The influence-matrix geometry predicted, \emph{before any learning ran},
    exactly where every method would fail. Injecting physics into the learner
    failed twice; using physics to know what is knowable succeeded.
  \end{alertblock}
\end{frame}
