...
\begin{frame}[t]{Five hypotheses, one confirmatory pass}
  ...
  \vspace{-1mm}
  \centering{\scriptsize\input{../report/tables/table_verdicts}}
  \vspace{3mm}
...
\begin{frame}[t]{H3 and H5 --- prognosis is where learning pays}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      ...
      \textbf{Two scales, and only one is a sales pitch:} $\sim$2.3$\times$ over the
      extrapolation an operator fields \emph{today}; over the best \emph{advanced} classical
      prognostic, $1.34\times$ at $90\,\%$ of life and a \alert{tie at $70\,\%$}
      ($p = 0.41$). The learning advantage is \alert{late-life}.
      \medskip
      ...
      \textbf{H5}: $\pm1957$ vs $\pm11\,509$ cy --- but the classical band was
      \alert{uncalibrated}. Conformalize both: $1.34\times$, not $6\times$.
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[t]{A certificate of what a diagnosis can know}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.44\textwidth}
      ...
      but the control, added later: $\approx0.24$ is the shared matrix, $\approx0.46$ survives
      an estimator that never sees $\mathbf{H}$, and ten directions cannot reach $p<0.05$ ---
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}\small
        \item prognosis: $858$ vs $1981$ cycles ($\sim$2.3$\times$ over fielded practice),
          robust across architectures and sensor quality --- but only $1.34\times$ over the
          best classical method, and \alert{only late in life}
        \item detection timing: $12\times$ earlier at equal false alarms
        \item calibrated uncertainty: $1.34\times$ tighter at equal coverage --- the
          $6\times$ headline was mostly conformal, which is free to both sides
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
...
\begin{frame}[t]{Appendix --- the compute budget}
  \centering{\tiny\input{../report/tables/table_compute}}
  \vspace{3mm}
