...
cuatro resultados más útiles. De hecho las dos refutaciones hicieron más trabajo
que las tres confirmaciones: las confirmaciones dijeron más o menos lo que el
campo esperaba --- el aprendizaje predice mejor la vida, detecta antes, cuantifica
mejor la incertidumbre. Son las refutaciones las que cambiaron nuestras
recomendaciones. Las voy a tomar en orden.
\end{decir}
...
\escena{di las dos escalas: no cites solo la grande} Y sé honesto sobre contra qué
línea base es eso. El factor 2,3 es contra la extrapolación lineal que un operador
tiene hoy desplegada. Contra el mejor prognóstico clásico avanzado --- basado en
similitud, que sí puede seguir la curva --- se estrecha a 1,34 veces. Y se estrecha
más todavía, porque volvimos atrás y corrimos la comparación en \textbf{todas} las
fracciones de vida en vez de solo al noventa por ciento. Al setenta por ciento de la
vida los dos métodos \textbf{empatan}: 1059 ciclos contra 1064, con una $p$ de 0,41.
La ventaja del aprendizaje en prognosis no es un desplazamiento uniforme a lo largo
de la vida. Es un efecto \textbf{de vida tardía}. Y tiene sentido: nuestro propio
piso prognóstico dice que a media vida el ochenta y siete por ciento de la dispersión
es irreducible. Donde no hay nada que ganar, no gana nadie.

Así que la frase honesta es: compra el horizonte de visita a taller, no la tendencia
de media vida.
...
\escena{la corrección: dila tú, no dejes que la encuentre otro} Pero ese factor seis
no es un resultado de IA, y quiero decirlo antes de que nadie lo pregunte. Nuestro
lado estaba calibrado con conformal al noventa por ciento. El suyo no estaba
calibrado con nada, sobre-cubre, y un intervalo que sobre-cubre es ancho por
construcción. La predicción conformal es agnóstica al modelo --- el lado clásico
puede cogerla y ya está. Así que volvimos atrás y envolvimos el mismo procedimiento
conformal alrededor de los tres predictores. A igualdad de cobertura, el intervalo de
la IA es \textbf{1,34 veces} más estrecho que el del clásico avanzado, y 2,3 veces
más estrecho que la práctica desplegada. No seis. Unas tres cuartas partes de aquel
titular eran el método de calibración, no el modelo.

Y la versión práctica de eso, para quien aquí tenga un prognóstico en producción:
antes de comprar un modelo, ponle un envoltorio conformal al que ya tienes. Cincuenta
líneas de código, sin tocar el modelo, y te llevas la mayor parte de la mejora del
intervalo.
\end{decir}
...
Prognosis --- 2,3 veces mejor que lo que hay hoy desplegado, y robusto entre tres
arquitecturas y a lo largo de la calidad de sensor. Contra el mejor método clásico la
ventaja es de 1,34 veces y solo tarde en la vida. Instante de detección.
Incertidumbre calibrada, 1,34 veces cuando los dos lados se calibran igual.

Es una afirmación más estrecha que la que habría hecho hace dos semanas, y es más
estrecha porque fuimos a buscar la debilidad de nuestros propios números de titular.
...
escalas: 2,3 veces mejor que lo que los operadores tienen hoy desplegado, y 1,34
veces mejor que el mejor método clásico. Una ventaja del treinta y cuatro por ciento,
no un orden de magnitud. Y ni siquiera eso en todas partes: al setenta por ciento de
la vida los dos empatan. La ventaja del aprendizaje es de vida tardía.
\end{decir}
