...
it. The \emph{acquisition} figures below are
separate measurements and are unaffected.
% ---------------------- PENDING: F24 not reflected here (minor) ----------------------
% This paragraph is CORRECT but stops at F23. F24 (sec:f24-scale, prereg-v27) went on to
% test magnitude instead of ranking, and returned two further results:
%   * the bound understates achievable error 8.9x with exponent 0.382 — a SHAPE error, so
%     no single constant repairs it; after calibration 90 % of errors sit within 5.6x;
%   * the powered within-direction test fails its pre-declared gate, because the CRB
%     varies only 1.15 % between engines (every SynCFM56 engine flies the same profile).
% Net effect on this deck's message: NONE. "Does not clear a physics-free null" remains
% true and remains the operative sentence for this audience. F24 refines WHY, and moves
% the status from "unproven" to the sharper "unmeasurable ON THIS FLEET, and here is the
% fleet that would measure it" (sec:future-c8).
%
% JUDGEMENT CALL, deliberately left open: adding it costs a sentence and buys a stronger
% position (a named next experiment beats an open caveat with a sceptical audience), but
% this is a 2-page brief for a performance boss, not an audit trail. Same call applies to
% oem-slides.tex and guia-presentacion-oem.tex, which carry the identical F23-era wording.
% Tracked: docs/TODO.md section 0, defect D4.
% -------------------------------------------------------------------------------------
