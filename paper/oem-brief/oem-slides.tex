...
\begin{frame}{An instrumentation design tool --- and it is not AI}
  ...
  ($\rho = 0.70$) does not clear its own physics-free null. A quarter is the matrix
  the bound shares with the estimator, half survives without it, and ten directions
  cannot reach significance --- not shown wrong, shown unprovable \emph{on this fleet},
  whose engines vary by $1.15\,\%$ in the quantity that had to vary. A mission-diverse
  fleet would settle it. The \emph{acquisition} numbers above are separate measurements
  and unaffected.}
\end{frame}
