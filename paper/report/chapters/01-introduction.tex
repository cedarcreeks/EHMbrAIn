...
\begin{enumerate}
  ...
  \item[\textbf{C8.}] \textbf{The earned-confidence certificate} (\cref{ch:breakthrough}): a
    per-engine, history-resolved identifiability guarantee for gas-path diagnosis. It states
    exactly which health directions an engine's own service history lets one conclude, and
    doubles as a sensor-acquisition instrument --- predicting, and this part is confirmed,
    that station probes rescue HPC efficiency $45\times$. Its \emph{honesty} --- whether
    certified precision really orders achievable error --- is the contribution's second half
    and the one this document treats at length: an exploratory $\rho = 0.70$ became, under
    four controls the original test lacked, a decomposition ($\approx 0.24$ shared matrix,
    $\approx 0.46$ surviving an estimator that never sees it) in which neither part clears
    significance. \textbf{The certificate is not shown to be dishonest; it is shown to be
    unmeasurable on this fleet} --- ranking is capped at ten health directions, and the
    magnitude route fails a pre-declared gate because the bound varies $1.15\,\%$ between
    engines that all fly the same profile (\cref{sec:f23-decoupled,sec:f24-scale}). The
    experiment that would settle it is named in \cref{sec:future-c8}. Both the validation and
    its refutation are ones only a synthetic-truth benchmark can perform, which is the point.
