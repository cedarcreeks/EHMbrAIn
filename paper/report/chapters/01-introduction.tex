...
Over the last decade, machine learning has produced striking results in the adjacent research
...
fleet degradation generator with physically derived fault signatures (planned, \cref{ch:wip}).
...
\begin{enumerate}
  ...
  \item[\textbf{C6.}] A determinability map: a sensors $\times$ noise $\times$ data-size
    ablation showing \emph{when} each approach wins.
...
\Cref{ch:background} builds the technical foundations assuming no prior knowledge of jet
engines or GPA: how a turbofan works, what is measured on the wing, and the mathematics of gas
path diagnosis. \Cref{ch:methodology} describes the testbed architecture and the methodological
safeguards. \Cref{ch:model} documents the thermodynamic twin of the CFM56-7B26 and its
calibration against certified data ---the work completed to date--- with quantitative evidence
of every step. \Cref{ch:wip} summarizes the remaining phases and their acceptance criteria.
