...
\begin{quote}\itshape
This chapter in brief: a virtual CFM56-7B26 is built in pyCycle by adapting a validated NASA
example, calibrated against certified data (its fuel-flow predictions land within 3\,\% of
measured test-bed values), and then interrogated for the two artifacts everything else
consumes: the healthy-engine baseline decks and the influence coefficient matrix, whose
geometry quantifies exactly how blind the cockpit sensor set is.
\end{quote}

\keyidea{The twin is built only from certified public data (type-certificate limits and measured emissions-databank fuel flows), so its fuel-flow predictions land within 3\,\% of test-bed values it never saw --- a recipe any operator could replicate for any engine.}
...
(\code{high_bypass_turbofan.py}) with the full component graph, the turbine-cooling bleed
...
\begin{figure}[htbp]
  ...
  work extraction across stations 45 and 5. Auto-generated.}
  \label{fig:stations}
\end{figure}
...
A model that converges is not yet a model that is \emph{right}. Calibration is the step that
ties the twin to reality: adjusting its few free scalars until it reproduces measured
certification data it was not built to match. The philosophy first, then the two iterations
it took.
...
to certification testing, and free. Most academic engine models ignore this source (\cref{sec:provenance} characterizes both sources as the provenance chain's root).
...
\begin{figure}[htbp]
  ...
  blocking the milestone (\cref{sec:idle}). Auto-generated.}
...
\begin{figure}[htbp]
  ...
  degraded accuracy of generic maps far from their scaling point. Auto-generated.}
...
(\code{solve_anchors()}) and covered by the tests.
...
available maximum. The 50\,\% floor is deliberate: airline EHM trending consumes takeoff,
climb and stable-cruise reports, all high-power conditions; approach and idle are neither
trending conditions nor reliable model territory (\cref{sec:idle}). At each condition the
model first finds the maximum thrust available at the rating temperature (point
\code{OD_max}, throttled by $T_4$), then solves the part-power point at the commanded
fraction (\code{OD_pt}) ---the same two-point pattern used by the pyCycle reference example.
The rating temperature is fixed at $T_{4,\max} = \SI{3200}{\degree R}$, a rounded envelope
constant chosen from the calibrated takeoff anchor (imposing the certified rated thrust at SLS
required $T_4 \approx \SI{3175}{\degree R}$, \cref{tab:anchors}); a real FADEC modulates
its rating limits across the envelope, so this constant is a declared simplification ---
adequate here because the baseline uses \emph{fractions} of the local maximum, not its
absolute value.
...
computations are parallelized; on the Apple M5 reference machine the sweep takes $3.5$ minutes on all cores versus $10$ minutes
...
The result is stored as a table plus a scattered linear interpolator (Delaunay triangulation
with barycentric interpolation, SciPy's \code{LinearNDInterpolator}) in the four operating
coordinates, and its quality is measured by leave-one-out cross-validation: each grid point is
removed, predicted from all the others, and the relative error recorded ---
\cref{tab:deck-loo} reports the per-channel median, 95th percentile and maximum. Median errors
are a few tenths of a percent; the tail percentiles are dominated by envelope-corner points
whose interpolation simplices span across altitude blocks in raw physical coordinates.

\subsection{Corrected-parameter exponents and the corrected-space baseline}
\label{sec:corrected-baseline}

The fix for those tails is the one anticipated in \cref{sec:stations}: build the baseline in
\emph{corrected-parameter} space. The correction exponents are fitted on the deck itself
rather than copied from generic tables: per channel $Y$, a linear least-squares fit of
\begin{equation}
  \log Y \;=\; \underbrace{\textstyle\sum_{k=0}^{3} c_k \left(\log N1_c\right)^{k}}_{\text{power line}}
  \;+\; a\,\log\theta \;+\; b\,\log\delta,
  \label{eq:exponent-fit}
\end{equation}
where the cubic polynomial absorbs the dependence on the corrected speed (the ``power line'')
and the exponents $(a, b)$ capture whatever ambient dependence remains --- exactly the
quantities the correction $Y_{\mathrm{corr}} = Y/(\theta^a \delta^b)$ then removes. The fitted exponents
land squarely on their textbook values without having been told them (\cref{tab:corrected}):
fuel flow $a=0.63$, $b=1.02$ (handbooks quote $\theta^{0.5\text{--}0.7}\delta$); N2
$a=0.49$, $b\approx0$ (the $\sqrt{\theta}$ corrected-speed law); EGT $a=0.95$, $b\approx0$
(temperature ratio). This is a further independent consistency check of the model physics.

Re-running the leave-one-out audit on the corrected channels over (corrected N1, Mach)
collapses the tails by an order of magnitude (\cref{tab:corrected}): worst-case N2 error drops
from 9.7\,\% to 0.5\,\%, EGT from 24\,\% to 2.0\,\%, fuel flow from 91\,\% to 6.6\,\%. Two
remarks for honesty: net thrust is excluded ---it is not an EHM sensor channel (it comes from
the thrust-setting definition), and it does not collapse under a $\theta$/$\delta$ law at fixed
N1c because of its strong direct Mach dependence; and the residual corrected-space errors are
comparable to the sensor noise floor that the fleet generator injects ($\sigma_{\mathrm{EGT}}
\approx 0.3\,\%$, $\sigma_{W_f} \approx 0.5\,\%$). Baseline error is also common-mode: both EHM
pipelines consume the same baseline, so it does not bias the comparison.

\begin{table}[htbp]
  \centering\small
  \caption{Fitted correction exponents per channel and the leave-one-out error before (raw
  4-D coordinates) versus after (corrected space). Auto-generated.}
  \label{tab:corrected}
  \input{tables/table_corrected}
\end{table}
...
With a calibrated healthy engine in hand, the last F1 artifact is the map from
\emph{faults} to \emph{symptoms} --- the influence coefficient matrix that every diagnosis
in this document inverts. Building it means deliberately injecting each fault and recording
the response.
...
\Cref{ch:gpa-practice} takes this geometry and measures what it costs a working diagnosis.

The analysis is repeated over a six-point grid spanning the envelope ---both snapshot
conditions plus power, altitude, climb and hot-day variations (\cref{tab:icm-grid}). The
observability structure barely moves: the minimum cockpit signature angle stays between 1.2 and
1.4\,degrees and the confusable-pair count is constant at seven. The smearing problem is
therefore \emph{intrinsic to the cockpit sensor set}, not an artifact of one flight condition
---which both justifies interpolating the ICM over this grid in phase F3 and forecloses the
easy escape of ``diagnose somewhere else in the envelope''.

\begin{table}[htbp]
  \centering\small
  \caption{Observability of the cockpit ICM across the operating-point grid. Auto-generated.}
  \label{tab:icm-grid}
  \input{tables/table_icm_grid}
\end{table}
...
rests on the model's sensitivity structure (the ICM), not on its absolute values --- an
assertion that is no longer left as one: \cref{sec:f-icm} perturbs every calibration knob
inside its own contract tolerance and finds the observability verdict (rank, the
$1.3^{\circ}$ confusable pair, the sensor conclusion) invariant, while flagging the one part
that is \emph{not} robust --- the exact membership of the confusable list at its $15^{\circ}$
boundary.
