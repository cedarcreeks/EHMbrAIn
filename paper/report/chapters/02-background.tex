...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[font=\footnotesize,
      comp/.style={draw, minimum height=8mm, minimum width=11mm, align=center},
      lp/.style={comp, fill=blue!12},
      hp/.style={comp, fill=red!12},
      st/.style={font=\scriptsize, text=black!60}]
    \node[lp] (fan) {Fan};
    \node[lp, right=3mm of fan] (lpc) {Booster\\(LPC)};
    \node[hp, right=3mm of lpc] (hpc) {HPC};
    \node[comp, fill=orange!15, right=3mm of hpc] (comb) {Combustor};
    \node[hp, right=3mm of comb] (hpt) {HPT};
    \node[lp, right=3mm of hpt] (lpt) {LPT};
    \node[comp, right=3mm of lpt] (cn) {Core\\nozzle};
    \node[comp, above=4mm of cn] (bn) {Bypass\\nozzle};
    \draw[-{Stealth}] (-1.1,0) -- (fan.west) node[midway, above, st] {air};
    \draw[-{Stealth}] (fan.east) -- (lpc.west);
    \draw[-{Stealth}] (lpc.east) -- (hpc.west);
    \draw[-{Stealth}] (hpc.east) -- (comb.west);
    \draw[-{Stealth}] (comb.east) -- (hpt.west);
    \draw[-{Stealth}] (hpt.east) -- (lpt.west);
    \draw[-{Stealth}] (lpt.east) -- (cn.west);
    \draw[-{Stealth}] (fan.north east) .. controls +(0.6,0.35) .. ([yshift=0mm]bn.west)
      node[pos=0.35, above, st] {bypass stream};
    % station ticks
    \foreach \x/\lbl in {fan.west/2, lpc.west/25, hpc.east/3, comb.east/4, hpt.east/45, lpt.east/5}
      \node[st, below=5.5mm of \x] {\lbl};
    \node[st, below=11mm of comb, align=center]
      {LP spool (N1): \textcolor{blue!60}{fan + booster + LPT}\quad
