...
\keyidea{What transfers off this simulator is not the absolute numbers but the rankings, mechanisms and geometry --- which method wins where and why, and the observability structure any CFM56-class cockpit set shares. Every operational claim in this document is of that second, transferable kind, and \cref{sec:f20-ncmapss} now demonstrates it rather than asserting it: on NASA's C-MAPSS, a different engine in different software, the confusable pair reproduces at $0.66^{\circ}$ against our $1.3^{\circ}$, and opens the same way once station probes are present.}
...
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.44\textwidth}>{\raggedright\arraybackslash}p{0.18\textwidth}@{}}
  ...
  Can we certify what a diagnosis honestly knows? & \textbf{Yes}, validated against truth
    ($\rho=0.70$), but \textbf{under-powered}: a physics-free null reaches $0.242$ and
    $p=0.085$ & \cref{ch:breakthrough} (H10), \cref{sec:f21-port} \\
  ...
  Should the certificate be used to filter what a learner sees? & \textbf{No}: masking the
    estimate to the identifiable subspace \emph{hurts} ($-0.119$, $t=-3.72$, channel count
    matched). A precision bound is not an absence of information & \cref{sec:f19-cert} \\
  Is a bidirectional network's edge the direction or the extra parameters? &
    \textbf{The parameters, on a GRU}: $94$\,cy of RUL error at doubled budget, nothing
    at equal. The sign \emph{reverses} on an LSTM, and that comparison is confounded by
    GRU-tuned hyperparameters & \cref{sec:f18-bidir,sec:f18-lstm} \\
  Is the confusable wall a fact about turbofans or about our simulator? & \textbf{About
    turbofans}: on NASA's C-MAPSS the same pair is $0.66^{\circ}$ on cockpit-class channels
    and $20.9^{\circ}$ with full instrumentation, against our $1.3^{\circ}$ and
    $26.7^{\circ}$ & \cref{sec:f20-ncmapss} \\
  Does the certificate survive a physics-free control? & \textbf{Not conclusively, and now we
    know why}: of $\rho=0.70$, $\approx0.24$ is the matrix the bound shares with the estimator
    and $\approx0.46$ survives an estimator that never sees it --- but ten directions cannot
    reach significance ($p=0.085$, $0.100$) & \cref{sec:f21-port,sec:f23-decoupled} \\
  Can magnitude be tested instead of ranking? & \textbf{Not on this fleet}: the bound
    understates error $8.9\times$ with exponent $0.382$, and every engine flies the same
    profile so the CRB varies only $1.15\,\%$ between them & \cref{sec:f24-scale} \\
  Does the prognostic floor hold on external truth? & \textbf{Yes}: irreducible share
    $0.68$--$0.82$ on N-CMAPSS against our $0.87$--$0.88$ & \cref{sec:f21-port} \\
...
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.44\textwidth}>{\raggedright\arraybackslash}p{0.18\textwidth}@{}}
  ...
  Can two architectures be compared at one architecture's hyperparameters? & \textbf{No} ---
    all three LSTM arms return negative $R^2$ on tuning selected for a GRU; that measures the
    tuning, not the cell & \cref{sec:f18-lstm} \\
...
cannot know}, tested against the true component state ($\rho = 0.70$ between certified
...
\subsection{The one experiment that would settle C8}
\label{sec:future-c8}

Three tests have now failed to prove the certificate honest, and between them they say
precisely why (\cref{sec:f21-port,sec:f23-decoupled,sec:f24-scale}). The ranking statistic is
capped at ten health directions, which the physics fixes. The magnitude statistic needs the
bound to vary between engines, and in SynCFM56 it does not: every engine flies a near-identical
N1 profile, so the accumulated Fisher information is nearly the same for all of them and the
within-direction coefficient of variation is $1.15\,\%$.

Neither wall is about the certificate. Both are about the fleet.

\textbf{The experiment is a fleet with deliberate mission diversity} --- route mixes different
enough that engines earn materially different bounds. Then certified precision can be regressed
against achieved error across engines, with direction effects differenced out and a predictor
that actually moves. That is a generator change and a re-run, not new theory, and the leverage
is known to be there: \cref{ch:tomography} moved separability by $79\,\%$ by changing a single
report condition, so operating history has first-order influence on this geometry.

Until it is run, the honest statement is the one this document now carries everywhere the
number appears: \emph{the certificate is not shown to be dishonest, it is shown to be
unmeasurable on this fleet.}
