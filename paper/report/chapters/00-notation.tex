...
\begin{tabular}{@{}p{0.10\textwidth}p{0.36\textwidth}p{0.09\textwidth}p{0.36\textwidth}@{}}
  \toprule
  ACARS & aircraft--ground datalink (snapshot reports) & LPC & low-pressure compressor (booster) \\
  BPR & bypass ratio & LPT & low-pressure turbine \\
  CEA & NASA Chemical Equilibrium with Applications & MAE & mean absolute error \\
  CI & continuous integration & MOPA & multiple-operating-point analysis \\
  C-MAPSS & NASA turbofan simulation benchmark & N1, N2 & low-/high-pressure spool speeds \\
  CUSUM & cumulative-sum change detector & N-CMAPSS & successor benchmark to C-MAPSS \\
  ...
  GRU & gated recurrent unit (neural network) & SLS & sea-level static \\
  HPC & high-pressure compressor & SVD & singular value decomposition \\
  HPT & high-pressure turbine & TCDS & type-certificate data sheet \\
  ICM & influence coefficient matrix & TPE & tree-structured Parzen estimator \\
  ISA & international standard atmosphere & TSFC & thrust-specific fuel consumption \\
  LOO & leave-one-out (cross-validation) & WLS & weighted least squares \\
  \bottomrule
\end{tabular}
\end{center}
